...
exports.getAccessToken = function (appid, secret, authorizeCode) {
  return fetch.get(`${URLS.API_OAU_ACCESS_TOKEN}?appid=${appid}&secret=${secret}&code=${authorizeCode}&grant_type=authorization_code`)
}

exports.refreshAccessToken = function (appid, refreshToken) {
  return fetch.get(`${URLS.API_OAU_REFRESH_ACCESS_TOKEN}?appid=${appid}&grant_type=refresh_token&refresh_token=${refreshToken}`)
}

exports.verifyAccessToken = function (accessToken, openid) {
  return fetch.get(`${URLS.API_OAU_VERIFY_ACCESS_TOKEN}?access_token=${accessToken}&openid=${openid}`)
}
...
exports.getUserInfo = function (accessToken, openid, lang = 'zh_CN') {
  return fetch.get(`${URLS.API_OAU_GET_USER_INFO}?access_token=${accessToken}&openid=${openid}&lang=${lang}`)
}
